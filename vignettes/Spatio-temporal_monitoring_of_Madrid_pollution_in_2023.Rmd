...
```{r setup}
library(widepollution)  #call our package
...
In this section we construct our main processed data sources out of data retrieved from [Madrid Open Data portal](https://datos.madrid.es/portal/site/egob). Our source datasets consists of a **wide-format** monthly tables with hours on columns and measuring stations by type of pollutant by day on rows. We store all raw retrieved files for this project as **external data**. 

## II.0 Comments on `NOTE` arising in `R CMD check`
We are aware that a `R CMD check` command on this package will give:

1. A **NOTE on extdata folder size bigger than 1MB**. However, as we want to showcase the complete process followed through this project, we include the processing of these bigger raw files in our package. The data selection and customization made for the analysis and discussion subsections is done later on (in *Data inspection* section).

2. A **NOTE on presence of UTF-8 encoded characters**. We deliberately intend to include some non-ASCII characters in the names of places around Madrid, in respect of the data source original language. As this is project is intended as a mock project that could be aimed at an organization, we find it valuable to maintain this support. Furthermore, we make sure the encoding is correctly done through the processing by using `enc2utf8()`.
...
We iterate data extraction and an unpivot process through all the monthly files. Then we construct a single long-format dataframe. We perform processing of data to ensure appropriate data types and set a useful dataframe structure for further analyses.
...
#Extract a single day-of-year value as date
#year_data$date <- as.Date(paste0('2023-',year_data$month,'-',year_data$day))

head(year_data)
```

We incorporate the name of the pollutants from a metadata file containing descriptive information about them.
...
#Encoding in UFt non-ASCII characters
...
We can now get a glance of the proportion of "verified" samples from the whole dataset, following the metadata description available on the open portal:
...
We also verify that no `NULL` values are provided in the dataset:
...
For simplicity and leveraging the most complete datasets, we will carry on our analysis with the NOx data subset. Thus, we continue to inspect in more detail data from this particular pollutant. We should have, by station-month combination, a total of measurements equal to the number of days of that month $\times\ 24$ hours of the day. We know, by the wide structure of our initial dataset with 24 measurements per day-pollutant-station combination, that missing hours will not be the case, so we perform the inspection at the "day level" (division by $24$):
...
Inspecting this summary dataframe, we conclude that (for this NOx dataset) most stations possess complete records per month and, most important, missing dates spatially concentrate in few stations. Therefore, as the rest of sensor have measurements for those time slots, we can get proxy readings for those locations by interpolation methods. With this in mind, we carry on with our workflow and coment that, after this inspection, the `NOx` dataset is to be stored as data in our package for later use.

## II.2 Integration of stations locations

We continue by loading the stations data and locations (point geometries), defining the column `station_id` to perform the dataframes join when needed in the further workflow.
...
stations <- rename_columns(stations,
                           c('CODIGO_CORTO','ESTACION','LONGITUD','LATITUD'),
                           c('station_id','station_name','long','lat'))
stations <- stations[c('station_id','station_name','long','lat')]

stations$station_name|>enc2utf8() -> stations$station_name #Ensuring utf-8
...
We then join the dataframes and inspect the size of our complete yearly `NOx` dataset (in MBs).
...
To complement the previous processing and exhibit the handling of another data type (namely, a JSON file) the following steps integrate the Madrid neighborhoods to our workflow to provide more spatial context to the pollution data and extract valuable insights per neighborhood. We stress that the original dataset provides the neighborhoods' border linestring coordinates given in the `ETRS89` CRS, which has `meters` as units. This will be suitable in the furthcoming analyses. However, for consistency with the pollution dataset, here we reproject (for now) to `WGS84`.
...
neigh_metadata$NOMBRE|>enc2utf8() -> neigh_metadata$NOMBRE #Ensuring utf-8
...
First rendering of a plot for contextual and consistency verification of datasets:
...
Moreover, we also make sure that the most persistently missing data (in stations with `station_id` 11 and 35, as shown in the `Dataset inspection` section) is enclosed by nearby sensors:
...
In this section we perform the data analysis of the previously retrieved, transformed and constructed datasets. To showcase the possibility of our package to store datasets as R objects, we consume the main dataset as data stored inside our package and retrieved by the `package::dataset` call.
...
We continue performing data aggregation of our `NOx` dataset at the month level and perform interpolation through `Inverse Distsance Interpolation` per month.
...
grid <- madrid |> st_bbox() |>
  st_as_stars(dx=100) |> #100m resolution to ease computations
  st_crop(madrid)
...
We now take advantage of the `stars` object created to perform an analysis per neighborhood that can be useful for the authorities, in order to identify risk at that level. Explicitly, we compute the mean concentration of `NOx` per neighborhood throughout the year.
...
We can, for instance, inspect the evolution of concentration of `NOx` in the first half of the year by performing an adequate (vector) data cube subsetting:

```{r}
nox_neigh_month[,,1:6] |> plot()
```

We readily verify that the beginning of the year suffered of most critical levels of `NOx` pollution and that this pollutant tends to concentrate in the center to center-south corridor of the city, verifying what was commented in the `Introduction`. Morevoer, we can obtain time statistics per neighborhood, such as a broad year summary figure (mean value) of the `NOx` concentration:
...
plot(nox_mad_mean_year['mean'],main = 'Mean NOx concentration in Madrid (2023)')
```
...
most_polluted$geometry |>
  plot(col='red',add=TRUE)
```

## III.2 Hour data analysis

Now we perform a similar analysis, this time first aggregating the numeric data into hour-of-the-day slots across the year 2023:
...
Similar to the previous case, we obtain the hour-of-the-day `NOx` concentration figure by neighborhood for further discussion.
...
We can finally inspect the evolution of concentration of `NOx` along, for instance, an even afternoon of 2023 (from 13 to 20 hrs.) in the metropolitan area of Madrid:

```{r}
nox_neigh_hour[,,13:20] |> plot()
```

We clearly identify the rush hour pollution rise after 19:00 (when vehicles are heavily used). It represents a peak time slot where major concentration around the city can be found and, in this case, increasingly more towards the outskirts of the city. Interestingly, We can readily verify how two nuclei at the south and north of the city center start to build up as the afternoon passes, until an overall high level (over 50 $\mu g$ of `NOx` concentration) is registered around the center after 20:00.

It is also interesting to notice that, as expected, during working hours the major concentration develops at the center of the city. Moreover, we can identify a great dispersion of `NOx` concentration values during the peak hours in the following graph:
...
We now take a closer look to the time series of the 10 overall most polluted neighborhoods. We verify that these neighborhoods show `NOx` concentration values that are similar to those of the upper part of the value intervals of the previous graph during the peak hours (around 9:00 and after 19:00):
...
Therefore, we notice that these 10 polluted neighborhoods not only show a strong `NOx` concentration as a whole during 2023 (yearly mean value), but also show tougher increases around the rush hours. Explicitly, for these 10 neighborhoods located in the centre-south of the city, the average `NOx` concentration from 19:00 to 21:00 is:
